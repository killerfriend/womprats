...
\begin{tabular}{|p{1.3in}|p{3in}|p{.8in}|p{.5in}|}
  \hline
  \textbf{Test Case Name:} & Fuse Test & \textbf{Test ID \#:} & 1\\
  \hline
  \textbf{Description:} & The fuse test verifies the power supply will burn the fuse and stop supplying power if the
current exceeds 2A. & \textbf{Type:} & blackbox \\
  \hline
  \multicolumn{4}{|l|}{\textbf{Tester Information}} \\ 
  \hline
  \textbf{Name of Tester:} & & \textbf{Date:} & \\
  ...
  \textbf{Hardware Ver:} & T-16 v1.0 & \textbf{Time:} & \\
  \hline
  \textbf{Setup:} & \multicolumn{3}{l|}{Properly bias the power supply and ensure it is outputting 3.3 VDC.} \\
  \hline
\end{tabular}
